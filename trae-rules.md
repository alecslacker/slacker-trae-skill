...
8. Memory Protocol — Memory = native TRAE, path resmi: `~/.trae/memory/user_profile.md` (global) + `~/.trae/memory/projects/<kunci>/project_memory.md` (project). Native inject otomatis; agent tidak membaca ulang kecuali diminta. DILARANG memory di folder project — bila ketemu, TANYA user dulu sebelum migrasi. Tulis sparse saat checkpoint: keputusan teknis, preferensi terkonfirmasi, progress, workaround. Entri ≤400 char, maks 20/file, >15 entri = pangkas. Detail: rules/memory-protocol.md
